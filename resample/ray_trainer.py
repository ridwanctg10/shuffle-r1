...
def compute_advantage(data: DataProto, adv_estimator: AdvantageEstimator, gamma: float = 1.0, lam: float = 1.0, purning_ratio: float = 0.5, experience_replay: bool = False, ablation_type: str = None):
    ...
    # # hack here to add ablation settings
    # if ablation_type is not None:
    #     adv_estimator = AdvantageEstimator.ABLATION
...
class RayPPOTrainer:
    ...
    def experience_resampling(self, batch, slice_sample=False):
        ...
        if self.config.trainer.sampling_weight == 'sum':
            adv_weights = np.array([item['adv_sum'] for item in batch])
        elif self.config.trainer.sampling_weight == 'max':
            adv_weights = np.array([item['adv_max'] for item in batch])
        elif self.config.trainer.sampling_weight == 'mean':
            adv_weights = np.array([item['adv_mean'] for item in batch])
        elif self.config.trainer.sampling_weight == 'pos':
            ...
            for batch_dict in tqdm(self.train_dataloader, desc="Running step", position=1):
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("reward", timing_raw):
                        ...
                        # add calculation about accuracy std
                        if "reward/accuracy" in reward_metrics.keys():
                            reward_update_metrics['reward/accuracy/std'] = np.std(reward_metrics["accuracy"])
                    ...
                    if self.config.trainer.experience_replay and self.config.algorithm.adv_estimator in [AdvantageEstimator.PAIRWISE_PURNING, AdvantageEstimator.PAIRWISE_FILTER_OVL_PURNING, AdvantageEstimator.PAIRWISE_PURNING_ADV_SHIFT]:
                    ...
                    assert isinstance(batch, DataProto), "type of data batch must be `DataProto`"
